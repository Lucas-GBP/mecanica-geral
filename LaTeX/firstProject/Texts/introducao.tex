...
\end{equation}
Contudo, nem todo caso é simples de encontrar a solução analítica. As forças podem depender de $n$ parâmetros, desde o tempo até a própria aceleração da partícula. Isso faz necessário técnicas mais avançadas com métodos de resolução de EDOs, onde situações que parecem simples a primeira vista, se tornam monstruosas \ref{eq:forca-dificil} \ref{eq:resposta-forca-dificil}:
\begin{equation}
...
É essa situação que será tratada nesse trabalho, uma solução numérica, utilizando uma simulação computacional para resolver um problema que seria muito trabalhoso, ou até impossível, de resolver algebricamente. O problema em questão é o lançamento de um projétil, onde a força de arrasto é dependente da posição e velocidade do projétil, e a força gravitacional é constante.
